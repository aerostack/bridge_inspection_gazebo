...
tmux new-window -t $SESSION:15 -n 'viewer + teleop'
...
tmux split-window -t $SESSION:16
...
tmux new-window -t $SESSION:17 -n 'pixhawk interface'
...
tmux new-window -t $SESSION:18 -n 'Basic Quadrotor Behaviors'
tmux send-keys "roslaunch basic_quadrotor_behaviors basic_quadrotor_behaviors.launch --wait \
  namespace:=drone$NUMID_DRONE" C-m

tmux new-window -t $SESSION:19 -n 'Quadrotor Motion With PID Control'
...
tmux new-window -t $SESSION:20 -n 'GroundTruth Gazebo'
...
tmux new-window -t $SESSION:21 -n 'Throttle_Controller'
tmux send-keys "roslaunch thrust2throttle_controller thrust2throttle_controller.launch --wait \
  namespace:=drone$NUMID_DRONE \
  uav_mass:=$UAV_MASS" C-m

tmux new-window -t $SESSION:22 -n 'Python interpreter'
...
tmux new-window -t $SESSION:23 -n 'Behavior Coordinator'
